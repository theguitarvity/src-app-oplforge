...
export function OnlineSourcesPage() {
  ...
  const addMutation = useMutation({
    mutationFn: (file: RemoteFile) => {
      ...
      if (!file.magnetUri)
        throw new Error('Esta fonte ainda não oferece um magnet seguro para a fila durável.')
    ...
    <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
      <Card className="h-fit">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Pesquise fontes remotas configuradas e adicione arquivos suportados à fila P2P.
        </p>
        ...
        {!selected ? (
          <div className="grid min-h-96 place-items-center text-center text-muted-foreground">
            <div>
              ...
              Selecione um resultado para ver detalhes e arquivos.
            </div>
          </div>
        ) : null}
        {selected ? (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {file.kind} - {file.format ?? 'sem formato'} - {formatBytes(file.size ?? 0)}
                    </p>
                  </div>
                  <Button
                  ...
                  >
                    <Download className="size-4" /> Adicionar à fila
                  </Button>
                </div>
              ))}
            </div>
