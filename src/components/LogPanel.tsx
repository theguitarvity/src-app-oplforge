    ...
    <div className="border-t border-white/10 bg-black/30 px-6 py-3 backdrop-blur-xl">
      <div className="mb-2 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-medium text-white/80">
          <Terminal className="size-4 text-violet-200" /> Logs
        </div>
        <Button variant="ghost" size="sm" onClick={clearLogs}><Trash2 className="size-4" /> Limpar</Button>
      </div>
      <div className="h-28 overflow-auto rounded-xl border border-white/10 bg-black/40 p-3 font-mono text-xs">
        {logs.length === 0 ? <p className="text-muted-foreground">Aguardando operacoes...</p> : null}
        {logs.map((log) => (
