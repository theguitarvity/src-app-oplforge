      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[85vh] w-[min(92vw,560px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-3xl border border-white/10 bg-[#12111b] p-6 shadow-glow">
          <Dialog.Title className="text-xl font-semibold text-white">
            Adequar nomes ao padrão OPL
          </Dialog.Title>
          <Dialog.Description className="mt-2 text-sm text-muted-foreground">
            Os Game IDs e títulos já foram reconhecidos automaticamente. Confirme para aplicar os
            nomes exibidos na prévia, sem copiar ou alterar o conteúdo dos jogos.
          </Dialog.Description>
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-3">
              <p className="text-2xl font-semibold text-emerald-200">{plan.itemIds.length}</p>
              <p className="text-xs text-muted-foreground">renomes selecionados</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              <p className="text-2xl font-semibold text-white">{plan.exclusions.length}</p>
              <p className="text-xs text-muted-foreground">itens preservados</p>
            </div>
          </div>
          {plan.exclusions.length > 0 ? (
            <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-400/5 p-3">
              <p className="text-sm font-medium text-amber-100">Itens que não serão alterados</p>
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            >
              {pending ? 'Adequando nomes…' : 'Confirmar adequação'}
            </Button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
