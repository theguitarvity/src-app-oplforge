...
| `boundAddresses` | `string[]`                                       | Every locally-relevant address the server is reachable on (FR-004). **Design note (post-implementation)**: a host can have more than one active local subnet at once (observed on this project's own dev machine — a secondary router adds a second `192.168.x.0/24` alongside the primary one). Both servers now bind `0.0.0.0` (all interfaces) rather than one hardcoded address, with `local-network-guard.ts` doing the actual FR-006 enforcement at accept/login time regardless of which interface a connection arrives on — so binding "everywhere" carries no additional exposure. `boundAddresses` is `listLocalNetworkAddresses()` computed once at start time, shown to the user so they can pick whichever one matches their PS2's subnet.                                        |
